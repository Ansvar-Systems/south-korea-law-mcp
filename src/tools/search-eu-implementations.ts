/**
 * search_eu_implementations — Search EU directives/regulations with Korean implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_korean_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS korean_statute_count,
  ...
  if (input.has_korean_implementation !== undefined) {
    sql += input.has_korean_implementation ? ` HAVING korean_statute_count > 0` : ` HAVING korean_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    korean_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        korean_statute_count: r.korean_statute_count,
