/**
 * validate_eu_compliance — Check Korean statute's EU/GDPR compliance status.
 */
...
export async function validateEUCompliance(
  ...
  if (input.provision_ref?.trim()) {
    ...
    const row = db.prepare(
      'SELECT id FROM legal_provisions WHERE document_id = ? AND (provision_ref = ? OR provision_ref = ? OR section = ? OR section = ?) LIMIT 1'
    ).get(resolvedId, input.provision_ref, articleRef, input.provision_ref, koreanRef) as { id: number } | undefined;
  ...
  if (rows.length === 0) {
    recommendations.push(
      'No EU references found. If this statute relates to EU law, consider adding cross-references.'
    );
  ...
  return {
    results: {
      ...
      adequacy_note: 'South Korea does not have an EU GDPR adequacy decision as of 2026. Adequacy talks have been ongoing since 2022.',
      warnings,
